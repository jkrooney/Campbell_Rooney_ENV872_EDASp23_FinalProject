...
wy_powerplants_sf <- 
  wy_powerplants_sf %>% 
  mutate(FuelType = case_when("CONVENTIONAL STEAM COAL" ~ "FossilFuel")), 
  ...
  #Unknown = "ALL OTHER",
                      #"NOT AVAILABLE",
                     #"OTHER GASES"))
#^This currently generates error, "Error in `stopifnot()`: ! Problem while computing `FuelType = case_when("CONVENTIONAL STEAM COAL" ~ "FossilFuel")`. Caused by error in `case_when()`: ! LHS of case 1 (`"CONVENTIONAL STEAM COAL"`) must be a logical vector, not a character vector.
