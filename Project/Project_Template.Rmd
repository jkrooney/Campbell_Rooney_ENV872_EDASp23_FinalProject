...
```{r wrangling average wind by county}
...
```{r spatial visualization of counties by poverty rate, include=FALSE}
#View with mapview
mapview(counties_sf_join, 
        zcol = 'E_POV150')
```

```{r spatial visualization of HUC 12 data}
...
```{r scatterplot visualization poverty by county, include=FALSE}
...
```{r percent poverty by county, include=FALSE}
...
```{r map of HUC12 by county}
#map newly created dataframe of HUC12Counties
mapview(HUC12Counties)
```

```{r visualize average wind by county as scatter}
